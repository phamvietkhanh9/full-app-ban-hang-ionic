...
class AdminController extends Controller {
	...
	*/
	public function login(Request $request)
	{
		...
		{
			return Redirect::to(env('user').'/home')->with('message', 'Chào mừng! Bạn đã đăng nhập hệ thống.');
		}
		else
		{
			return Redirect::to(env('user').'/login')->with('error', 'Tên đăng nhập và mật khẩu không đúng')->withInput();
		}
	}
	...
	*/
	public function logout()
	{
		...
		return Redirect::to(env('user').'/login')->with('message', 'Đăng xuất khỏi hệ thống !');
	}
	...
	*/
	...
	{		
		...
		return Redirect::back()->with('message','Thông tin tài khoản Đã cập nhật thành công.');
	}
