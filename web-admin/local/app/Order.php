...
class Order extends Authenticatable
{
   ...
   public function addNew($data)
   {
      ...
      //send sms to admin
      $msg = "Đơn hàng mới đã nhận #".$add->id." với tổng số tiền ".$admin->currency.$add->total;

      app('App\Http\Controllers\Controller')->sendSms($admin->phone,$msg);

      //send sms to user
      $msg2 = "Kính gởi ".$add->name.", Đơn hàng của bạn đã được đặt thành công. Mã đơn hàng của bạn là #".$add->id."\n Tổng số tiền ".$admin->currency.$add->total;

      app('App\Http\Controllers\Controller')->sendSms($add->phone,$msg2);

      app('App\Http\Controllers\Controller')->sendPushS("Đơn hàng mới đã nhận",$msg,$add->user_id);
   ...
   public function history($id)
   {
      ...
      foreach($orders as $order)
      {
         ...
         {
            $status = "Lấy hàng lúc ".$order->status_time;
         }
   ...
   public function getStatus($id)
   {
      $order = Order::find($id);
         ...
         if($order->status == 0)
         {
            $status = "<span class='badge badge-soft-danger badge-light'>Đang xử lý</span>";
         }
         elseif($order->status == 1)
         {
            $status = "<span class='badge badge-soft-info badge-light'>Đã xác nhận</span>";
         }
         elseif($order->status == 2)
         {
            $status = "<span class='badge badge-soft-warning badge-light'>Đã huỷ</span>";
         }
         elseif($order->status == 3)
         {
            $status = "<span class='badge badge-soft-info badge-light'>Chỉ định đối tác giao hàng</span>";
         }
         elseif($order->status == 4)
         {
            $status = "<span class='badge badge-soft-info badge-light'>Đang đi giao hàng</span>";
         }
         else
         {
            $status = "<span class='badge badge-soft-success badge-light'>Đã giao hàng</span>";
         }
   ...
   public function sendSms($id)
   {
      ...
      if($order->status == 1)
      {  
         $msg = "Kính gởi ".$order->name.", đơn hàng của bạn số #".$order->id." đã được xác nhận. Tổng số tiền đặt hàng là ".$admin->currency.$order->total;
         $title = "Xác nhận đặt hàng";
      }
      elseif($order->status == 2)
      {
         $msg   = "Kính gởi ".$order->name.", đơn hàng của bạn số #".$order->id." đã bị hủy.";
         $title = "Đơn hàng đã bị hủy";

      }
      elseif($order->status == 3)
      {
         $msg = "Kính gởi ".$order->name.", Đối tác giao hàng đã được chỉ định cho đơn đặt hàng của bạn số.".$order->id;
         $title = "Đối tác giao hàng được chỉ định";

         $msg2 = "Đơn hàng mới số #".$order->id." đã nhận được, nhấn để biết thêm thông tin";
         app('App\Http\Controllers\Controller')->sendPushD("Đơn hàng mới đã nhận",$msg2,$order->d_boy);

      }
       elseif($order->status == 4)
      {
         $msg = "Kính gởi ".$order->name.", đơn hàng của bạn số #".$order->id." đang trênd dường vận chuyển.";
         $title = "Đơn hàng đang trên đường giao hàng";
      }
      else
      {
         $msg = "Kính gởi ".$order->name.", đơn hàng của bạn số #".$order->id." đã giao thành công.";
         $title = "Giao hàng";
   ...
   public function getUnit($id)
   {
      ...
      if($item->small_price)
      {
         $data[] = ['id' => 1,'name' => "Nhỏ: ".$item->small_price];
      }

      if($item->medium_price)
      {
         $data[] = ['id' => 2,'name' => "Giá bán: ".$item->medium_price];
      }

      if($item->large_price)
      {
         $data[] = ['id' => 3,'name' => "Lớn: ".$item->large_price];
      }
