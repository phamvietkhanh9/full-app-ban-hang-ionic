...
@ViewChild('directionsPanel',{static:false}) directionsPanel: ElementRef;
 ...
 address:any;
  ...
  getStatus()
  {
    ...
    if(this.status == 2)
    {
      this.presentToast("Lấy làm tiếc! Đơn đặt hàng của bạn đã bị hủy.");
    ...
    if(this.status == 5)
    {
      this.presentToast("Đơn đặt hàng đã được giao thành công");
  ...
  async loadMap(dboy) {
    ...
    this.geolocation.getCurrentPosition().then((resp) => {
      ...
      console.log('Lỗi khi nhận vị trí', error);
    });
  }
   ...
   async getAddressFromCoords(lattitude, longitude) {
    ...
    this.nativeGeocoder.reverseGeocode(lattitude, longitude, options)
      ...
      .catch((error: any) =>{ 
        this.address = "Địa chỉ không có sẵn!";
      });
