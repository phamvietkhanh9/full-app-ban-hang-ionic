...
export class CheckoutPage implements OnInit {
  ...
  ionViewWillEnter()
  {
    if(!localStorage.getItem('user_id') || localStorage.getItem('user_id') == 'null')
    {
      ...
      this.presentToast("Vui lòng đăng nhập để tiếp tục.");
    }
    else
    {
      this.loadData();
  ...
  async order()
  {
    ...
    var allData = {user_id : localStorage.getItem('user_id'),payment : this.payment,address : this.address,cart_no : localStorage.getItem('cart_no'),payment_id : this.payment_id,otype : this.otype,notes:this.notes}
  ...
  { 
        ...
        this.payPal.prepareToRender('PayPalEnvironmentSandbox', new PayPalConfiguration({
          ...
          let payment = new PayPalPayment(this.total_amount, 'USD', 'Sự miêu tả', 'sale');
          this.payPal.renderSinglePaymentUI(payment).then((res) => {
            ...
            this.presentToast("Giao dịch Paypal bị hủy");

          });
        }, () => {

          this.presentToast("Lỗi cấu hình");

        });
      }, () => {
        // 
          this.presentToast("Lỗi khi khởi chạy, có thể PayPal không được hỗ trợ");
  ...
  payWithStripe()
  {
    if(this.card_no.length > 10 && this.exp_month && this.exp_year && this.cvv)
    {
        ...
        this.stripe.createCardToken(card)
          ...
          this.presentToast("Vui lòng nhập chi tiết thanh toán hợp lệ");
  ...
  async makePayment(token)
  {
    ...
    {
      this.presentToast("Đã xảy ra lỗi. Vui lòng thử lại.");
    }
